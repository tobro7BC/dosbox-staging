...
{
public:
	MsgLocation(const std::string& file_name, const size_t line_number) : 
		file_name(file_name),
		line_number(line_number) {}

	bool operator==(const MsgLocation& other) const;
...
void MSG_Add(const std::string& message_key,
             ...
             const MsgLocation& location = { __builtin_FILE(), __builtin_LINE() });
