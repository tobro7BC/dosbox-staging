...
{
	return GetLocation() == MetadataKey;
}
...
PoReader::PoReader(const std_fs::path& file_path) : file_name(file_path.string())
{
	if (file_path.empty()) {
		return;
	}
...
void PoReader::LogWarning(const int line_number,
...
std::string PoReader::ReadSingleLineString(const std::string& line) const
{
	...
	for (size_t idx = 1; idx + 1 < line.size(); ++idx) {
		...
		case '"':  result += '"';  break;
		default:
			LogWarning(line_counter, format_str("unsupported escaping character 0x%02x", line[idx]));
			result += '\\';
			result += line[idx];
			break;
		}
...
void PoReader::ReadFlags(const std::string& line)
{
	...
	for (auto flag : split(content, ",")) {
		trim(flag);
		if (HasFlag(flag)) {
			LogWarning(line_counter, format_str("duplicated flag '%s'", flag.c_str()));
		} else {
			AddFlag(flag);
		}
	}
...
bool PoReader::ValidateGettextMetadata() const
{
	...
	if (!IsFirstEntry()) {
		LogWarning("only the first entry is expected to contain "
			   "gettext metadata");
		return true;
	}
	...
	for (const auto& entry : split(GetTranslated(), "\n\r")) {
		...
		for (auto value : split(values, ";")) {
			...
			if (value != "charset=UTF-8" && !found_non_utf8) {
				found_non_utf8 = true;
				LogWarning("gettext metadata indicates "
...
bool PoWriter::WriteHeader(const std::string& language)
{
	...
	auto add = [&header_data](const std::string &key,
	...
	add("X-Generator",               std::string(DOSBOX_NAME " ") +
		                         DOSBOX_GetVersion());
...
bool PoWriter::WriteDosBoxMetadata(const std::string& key,
                                   const std::string& value,
                                   ...
                                   const bool is_fuzzy)
{
	...
	AddHelpLine("Do not translate this, set according to the instruction!");
