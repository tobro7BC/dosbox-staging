...
static DosBox::Rect get_canvas_size_in_pixels()
{
	...
	case RenderingBackend::Texture:
		if (SDL_GetRendererOutputSize(sdl.renderer,
			...
			SDL_GetWindowSize(sdl.window, &canvas_size_px.w,
					&canvas_size_px.h);
		}
		break;
...
static bool update_textures()
{
	...
	if (SDL_MUSTLOCK(sdl.texture.curr_framebuf)) {
		SDL_LockSurface(sdl.texture.curr_framebuf);
	}
	if (SDL_MUSTLOCK(sdl.texture.last_framebuf)) {
		SDL_LockSurface(sdl.texture.last_framebuf);
	}
...
static void switch_fullscreen()
{
	...
	if (!sdl.desktop.is_fullscreen) {
		sdl.desktop.window.canvas_size = to_sdl_rect(get_canvas_size_in_pixels());
	}
...
static void shutdown_gui(Section*)
{
	...
	// Let SDL cleanup up after itself (exit fullscreen, restore mouse state,
	// etc.) instead of us attempting to do it manually. That is unnecessary
	// and a moving target with each SDL library upgrade.
	//
	// Similarly, all GPU resources (textures, compiled shaders, etc.) will be
	// cleaned up by the driver automatically. That's the best way, we don't
	// need to attempt to do a manual cleanup.
}
...
static void set_output(Section* sec, const bool wants_aspect_ratio_correction)
{
	...
	if (sdl.want_rendering_backend == RenderingBackend::OpenGl) {
		if (!create_gl_renderer()) {
			...
			// We don't need a render reinit here when starting in OpenGL
			// mode. But when starting in `texture` mode and later switching
			// to `opengl` output, the function `GFX_SetShader()` wouldn't get
			// called which will result in no shader being loaded and a black
			// screen.
			//
			// It's pretty hard to get around this due to how shader
			// auto-switching works, so the cleanest and simplest way is to
			// always do a render reinit here. Doing the init twice in the
			// normal `output = opengl` case has no ill effects.
			//
			RENDER_Reinit();
			return;
		}
...
static void init_sdl_config_section()
{
	...
	auto pint = sdl_sec->AddInt("display", OnlyAtStart, 0);
	pint->SetHelp(
	...
	pstring = sdl_sec->AddString("fullresolution", DeprecatedButAllowed, "");
	pstring->SetHelp(
	...
	pint = sdl_sec->AddInt("transparency", Deprecated, 0);
	pint->SetHelp(
	        "Renamed to [color=light-green]'window_transparency'[reset].");

	pint = sdl_sec->AddInt("window_transparency", Always, 0);
	pint->SetHelp(
	...
	pbool = sdl_sec->AddBool("pause_when_inactive", OnlyAtStart, false);
	...
	pstring = sdl_sec->AddString("screensaver", OnlyAtStart, "auto");
	pstring->SetHelp(
