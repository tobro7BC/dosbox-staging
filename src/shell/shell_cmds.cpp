...
const std::map<std::string, SHELL_Cmd> DOS_Shell::shell_cmds = {
	...
	{ "GOTO",     {1, &DOS_Shell::CMD_GOTO,     "GOTO"} },
	{ "IF",       {1, &DOS_Shell::CMD_IF,       "IF"} },
...
bool lookup_shell_cmd(std::string name, SHELL_Cmd &shell_cmd)
{
	...
	const auto result = DOS_Shell::shell_cmds.find(name);
	if (result == DOS_Shell::shell_cmds.end())
...
bool DOS_Shell::execute_shell_cmd(char *name, char *arguments) {
	...
	std::string tmp = std::regex_replace(
	        arguments, re, "$1/?$3", std::regex_constants::format_first_only);
