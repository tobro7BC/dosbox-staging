...
static void FPU_FPREM1(void)
{
	...
	int64_t ressaved;
	if (quot - quotf > 0.5) {
		ressaved = static_cast<int64_t>(quotf + 1);
	} else if (quot - quotf < 0.5) {
		ressaved = static_cast<int64_t>(quotf);
	} else {
		ressaved = static_cast<int64_t>(
		        (((static_cast<int64_t>(quotf)) & 1) != 0) ? (quotf + 1)
		                                                   : (quotf));
	}
	fpu.regs[TOP].d = valtop - ressaved * valdiv;
	FPU_SET_C0(static_cast<Bitu>(ressaved & 4));
